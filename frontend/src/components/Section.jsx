...
// Prevents duplicate <style> tags when Section
// renders multiple times (9x on a full teardown)
// ─────────────────────────────────────────────
...
function useInjectStyles(css) {
  useEffect(() => {
    ...
    tag.setAttribute("data-section-styles", "true");
    tag.textContent = css;
...
function ListItem({ children, index }) {
  return (
    <li
      className="section-list-item animate-fade-up"
...
function EmptyState({ message = "No data available." }) {
  return (
    <p className="section-empty">{message}</p>
  );
}
...
function CollapsibleCard({ title, badge, children, defaultOpen = true }) {
  ...
  const [bodyHeight, setBodyHeight] = useState(defaultOpen ? "auto" : "0px");

  // Measure real height for smooth transition
  useEffect(() => {
    if (!bodyRef.current) return;
    if (open) {
      // Set to actual scrollHeight first, then to "auto" after transition
      const h = bodyRef.current.scrollHeight;
      setBodyHeight(`${h}px`);
      const timer = setTimeout(() => setBodyHeight("auto"), 300);
      return () => clearTimeout(timer);
    } else {
      // Snapshot current height before collapsing (avoids jump from "auto")
      const h = bodyRef.current.scrollHeight;
      setBodyHeight(`${h}px`);
  ...
  return (
    <div className={`collapsible-card ${open ? "collapsible-card--open" : ""}`}>
      ...
      <div
        ref={bodyRef}
        className="collapsible-card__body-wrapper"
        style={{
...
function PainPoints({ data }) {
  if (!data?.length) return <EmptyState message="No pain points identified." />;
  return (
    <div className="section-content">
...
function CompetitiveLandscape({ data }) {
  const hasDirect   = data.direct_competitors?.length > 0;
  const hasIndirect = data.indirect_competitors?.length > 0;
      ...
      <div className="comp-grid">
        <div className="comp-column animate-fade-up delay-0">
          ...
          {hasDirect ? (
            ...
            <EmptyState message="No direct competitors identified." />
          )}
        </div>
        <div className="comp-column animate-fade-up delay-2">
          ...
          {hasIndirect ? (
            ...
            <EmptyState message="No indirect competitors identified." />
          )}
        </div>
      </div>
...
function WhatWorks({ data }) {
  if (!data?.length) return <EmptyState message="No strengths identified." />;
  return (
    <div className="section-content">
...
function WhatIsMissing({ data }) {
  if (!data?.length) return <EmptyState message="No gaps identified." />;
  return (
    <div className="section-content">
...
function RedFlags({ data }) {
  if (!data?.length) return <EmptyState message="No red flags identified." />;
  return (
    <div className="section-content">
...
// ─────────────────────────────────────────────
// Styles — injected once via useInjectStyles
// ─────────────────────────────────────────────
...
export default function Section({ sectionKey, data, index = 0 }) {
  ...
  // Inject styles once — no duplicate <style> tags across 9 renders
  useInjectStyles(SECTION_STYLES);
  ...
  return (
    ...
    >
      ...
      <div className="teardown-section__header">
        ...
        <button
          className="teardown-section__toggle"
